...
impl AotContractExecutor {
    ...
    pub fn run(
        ...
        if cfg!(target_arch = "aarch64") {
            ...
            len_u32.to_bytes(&mut invoke_data)?; // cap
        } else if cfg!(target_arch = "x86_64") {
            (0u32 as u64).to_bytes(&mut invoke_data)?; // start
            (len_u32 as u64).to_bytes(&mut invoke_data)?; // end
            (len_u32 as u64).to_bytes(&mut invoke_data)?; // cap
        } else {
...
mod tests {
    ...
    // todo add recursive contract test See: https://github.com/starkware-libs/cairo_native/issues/1220

    #[fixture]
    fn starknet_program() -> ContractClass {
        load_contract("contracts/simple_storage_dup.contract.json")
    }

    #[fixture]
    fn starknet_program_factorial() -> ContractClass {
        load_contract("contracts/simple_storage_factorial.contract.json")
    }

    #[fixture]
    fn starknet_program_empty() -> ContractClass {
        load_contract("contracts/simple_storage_empty.contract.json")
    }
